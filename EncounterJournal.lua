...
function EncounterJournal_OnShow(self)
	...
	if ( EncounterJournal.instanceSelect.tier ) then
		EncounterJournal.instanceSelect.tier:SetText(EJ_TIERS[EJ_GetExpansion()]);
	end
end
...
function EncounterJournal_ListInstances()
	...
	local showRaid = not EncounterJournal.instanceSelect.raidsTab:IsEnabled()
		or EncounterJournal.instanceSelect.raidsTab:IsEnabled() == 0;
	...
	local rows = ceil(shown / EJ_NUM_INSTANCE_PER_ROW);
	self:SetHeight(max(rows * 111 + 20, 375));

	local scroll = EncounterJournal.instanceSelect.scroll;
	scroll:UpdateScrollChildRect();
	ScrollFrame_OnScrollRangeChanged(scroll, 0, max(rows * 111 + 20 - scroll:GetHeight(), 0));
end
...
function EncounterJournal_DisplayInstance(instanceID, noButton)
	...
	self.instance.loreScroll.child:SetHeight(self.instance.loreScroll.child.lore:GetStringHeight() + 10);
	...
	local bossCount = bossIndex - 1;
	EncounterJournal.encounter.infoFrame:SetHeight(max(bossCount * 70 + 20, 372));
	self.info.detailsScroll:UpdateScrollChildRect();
...
function EncounterJournal_DisplayEncounter(encounterID, noButton)
	...
	self.infoFrame.description:Show();
	...
	-- ask the server for all creatures of the encounter at once (models appear without delay)
	local encounterDisplayIDs = {};
	for i = 1, MAX_CREATURES_PER_ENCOUNTER do
		local _, _, _, creatureDisplayInfo = EJ_GetCreatureInfo(i);
		if creatureDisplayInfo then
			table.insert(encounterDisplayIDs, creatureDisplayInfo);
		end
	end
	EncounterJournal_PrefetchDisplayIDs(encounterDisplayIDs);
	for i=1,MAX_CREATURES_PER_ENCOUNTER do 
...
function EncounterJournal_DisplayCreature(self)
	...
	elseif self.displayInfo then
		-- boss model through the creature cache (EncounterJournalModel.lua)
		EncounterJournal_SetModelByDisplayID(EncounterJournal.encounter.model, self.displayInfo);
	...
	if hideHeaders then
		...
		while nextSectionID do
			...
			elseif not fileredByDifficulty then --ignore all sections that should not be shown with our current difficulty settings		
				...
				infoHeader.description:ClearAllPoints();
				infoHeader.description:SetPoint("TOPLEFT", infoHeader.button, "BOTTOMLEFT", 10, -9);
				...
				if displayInfo and displayInfo > 0 then
					local portrait = GetCreaturePortraitByDisplayID(displayInfo);
					if ( portrait ) then
						infoHeader.button.portrait.icon:SetTexture("Interface\\Icons\\"..portrait);
					else
						infoHeader.button.portrait.icon:SetTexture("Interface\\Icons\\INV_Misc_QuestionMark");
					end
		...
		if topLevelSection and usedHeaders[1] then
			usedHeaders[1]:SetPoint("TOPRIGHT", 0 , -8 - self.description:GetStringHeight() - SECTION_BUTTON_OFFSET);
		end
	end
	...
	if not doNotShift then
		...
		if self.index then
			...
			if self.description:IsShown() then
				anchorY = anchorY - self.description:GetStringHeight() - SECTION_DESCRIPTION_OFFSET;
			end
...
function EncounterJournal_ShiftHeaders(index)
	...
	for i=index,#usedHeaders-1 do
		...
		if usedHeaders[i].description:IsShown() then
			anchorY = anchorY - usedHeaders[i].description:GetStringHeight() - SECTION_DESCRIPTION_OFFSET;
		else
	...
	local last = usedHeaders[#usedHeaders];
	if ( last ) then
		local _, _, _, _, lastY = last:GetPoint();
		if ( lastY ) then
			local total = abs(lastY) + last:GetHeight() + 20;
			if ( last.description:IsShown() ) then
				total = total + last.description:GetStringHeight() + SECTION_DESCRIPTION_OFFSET;
			end
			EncounterJournal.encounter.infoFrame:SetHeight(max(total, 372));
		end
	end
end
...
function EncounterJournal_MoveSectionUpdate(self)
	...
	if self.frameCount > 0 then
		local _, _, _, _, anchorY = self:GetPoint();
		local height = min(EJ_MAX_SECTION_MOVE, self:GetHeight() + self.description:GetStringHeight() + SECTION_DESCRIPTION_OFFSET);
...
function EncounterJournal_LootUpdate()
	...
	for i = 1,#items do
		...
		if index <= numLoot then
			...
			local name, icon, slot, armorType, itemID, link, encounterID, quality = EJ_GetLootInfoByIndex(index);
			item.name:SetText(name);
			if ( quality and ITEM_QUALITY_COLORS[quality] ) then
				item.name:SetVertexColor(ITEM_QUALITY_COLORS[quality].r, ITEM_QUALITY_COLORS[quality].g, ITEM_QUALITY_COLORS[quality].b);
			else
				item.name:SetVertexColor(1, 0.82, 0);
			end
			item.icon:SetTexture(icon);
			item.slot:SetText(slot);
			item.armorType:SetText(armorType);
			local bossName = encounterID and EJ_GetEncounterInfo(encounterID);
			if ( bossName ) then
				item.boss:SetFormattedText(BOSS_INFO_STRING, bossName);
			else
				item.boss:SetText("");
			end
...
function EncounterJournal_OnSearchTextChanged(self)
	...
	if EncounterJournal.searchResults:IsShown() then
		...
		EncounterJournal.searchBox.sbutton1.botRightCorner:SetPoint("BOTTOM", button, "BOTTOM", 0, -5);
	end
end
...
function EJ_TierDropDown_Initialize(self)
	for i = 0, 2 do
		local info = UIDropDownMenu_CreateInfo();
		info.text = EJ_TIERS[i];
		info.value = i;
		info.checked = (EJ_GetExpansion() == i);
		info.func = function(button)
			EJ_SetExpansion(button.value);
			EncounterJournal.instanceSelect.tier:SetText(EJ_TIERS[EJ_GetExpansion()]);
			EncounterJournal_ListInstances();
			CloseDropDownMenus();
		end
