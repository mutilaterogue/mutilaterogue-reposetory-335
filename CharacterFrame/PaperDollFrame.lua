...
function Mastery_OnEnter(statFrame)
	...
	local mastery = GetCustomCombatRating(CR_MASTERY);
	local masteryBonus = GetCustomCombatRatingBonus(CR_MASTERY);

	local statLabel = STAT_MASTERY;
	local title = HIGHLIGHT_FONT_COLOR_CODE..format(PAPERDOLLFRAME_TOOLTIP_FORMAT, statLabel).." "..format("%.2F", mastery)..FONT_COLOR_CODE_CLOSE;
	...
	local masteryKnown = false;
	if (CLASS_MASTERY_SPELLS and class and CLASS_MASTERY_SPELLS[class] and IsSpellKnown) then
		masteryKnown = IsSpellKnown(CLASS_MASTERY_SPELLS[class]) and true or false;
	end

	local primaryTalentTree;
	if (GetPrimaryTalentTree) then
		primaryTalentTree = GetPrimaryTalentTree();
	else
		-- 3.3.5: fall back to the tree with the most points spent
		...
		primaryTalentTree = best;
	end

	local baseLine = format(STAT_MASTERY_TOOLTIP, format("%.2f", mastery - masteryBonus), format("%.2f", masteryBonus));

	if (masteryKnown and primaryTalentTree and GetTalentTreeMasterySpells) then
		...
		if (masterySpell) then
			...
			if (name) then
				...
				local desc = GetSpellDescription(masterySpell);
				if (desc) then
		...
		if (masterySpell2) then
			local name2 = GetSpellInfo(masterySpell2);
			if (name2) then
				GameTooltip:AddLine(" ");
				GameTooltip:AddLine(name2, 1, 1, 1);

				local desc2 = GetSpellDescription(masterySpell2);
				if (desc2) then
					GameTooltip:AddLine(desc2, NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b, true);
				end
			end
		end

		GameTooltip:AddLine(" ");
		GameTooltip:AddLine(baseLine, NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b, true);
	else
		GameTooltip:AddLine(baseLine, NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b, true);
		GameTooltip:AddLine(" ");
...
function PaperDollItemSlotButton_Update (self)
	...
	if ( textureName ) then
		...
		SetItemButtonQuality(self, nil); -- пустой слот: прячем рамку
		SetItemButtonTextureVertexColor(self, 1.0, 1.0, 1.0);
		SetItemButtonNormalTextureVertexColor(self, 1.0, 1.0, 1.0);
		if ( cooldown ) then
			cooldown:Hide();
