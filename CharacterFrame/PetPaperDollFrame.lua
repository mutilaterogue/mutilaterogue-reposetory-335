-- PetPaperDollFrame: Cataclysm 4.3.4 -> 3.3.5a
-- Спутники (питомцы/транспорт) убраны - они в «Коллекциях».
...
function PetPaperDollFrame_OnLoad (self)
	self:SetFrameLevel(CharacterFrame:GetFrameLevel() + 2);
...
function PetPaperDollFrame_UpdateIsAvailable()
	if ( (not HasPetUI()) ) then
		...
		CharacterFrame.TabSystem:SetTabShown(CharacterFrame.tabIDs["PetPaperDollFrame"], false);
		...
		CharacterFrame.TabSystem:SetTabShown(CharacterFrame.tabIDs["PetPaperDollFrame"], true);
	end
end
...
-- 3.3.5: UnitPopup.lua зовёт PetPaperDollFrame_SetTab(1); вкладок спутников больше нет
